...
class CocoStuffSegDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if os.path.exists(mask_path):
            mask = Image.open(mask_path)  # PNG 文件直接包含类别索引
            mask = self.mask_transform(mask) * 255  # 转换为类别索引
            mask = mask.squeeze(0).long()  # [H, W]
        else:
            mask = torch.zeros(self.img_size, dtype=torch.long)  # 默认空掩码
...
def calculate_iou(pred, target, num_classes):
    ...
    for cls in range(num_classes):
        ...
        if union == 0:
            ious.append(float('nan'))  # 如果没有该类，跳过
        else:
...
def test_seg(token=None):
    ...
    # 评估指标
    num_classes = 182  # COCO-Stuff 有 182 类
